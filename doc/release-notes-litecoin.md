Concoin Core version *0.15.1* is now available from:

  <https://download.concoin.org/concoin-0.15.1/>
...
A number of changes to the way Concoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 cons per kB (0.00001 CON). This is to prepare for dropping the minimum transaction fee to 0.1 cons per kB (0.0001 CON) in the next release.
